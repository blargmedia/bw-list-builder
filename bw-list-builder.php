...
/*
    ...
    5. actions
      ...
      5.16  bwlb_check_wp_version()
      5.17  bwlb_uninstall_plugin()
      5.18  bwlb_remove_plugin_tables()
      5.19  bwlb_remove_post_data()
      5.20  bwlb_remove_options()
    ...
    6. helpers
      ...
      6.23  bwlb_get_list_subscribers()
      6.24  bwlb_get_list_subscriber_count()
      6.25  bwlb_get_export_link()
      6.26  bwlb_get_admin_notice()
      6.27  bwlb_get_options_settings()
...
add_action('admin_notices', 'bwlb_check_wp_version');
register_uninstall_hook(__FILE__, 'bwlb_uninstall_plugin');
...
function bwlb_list_column_data ( $column, $post_id ) {
  ...
  switch ( $column ) {
    ...
    case 'subscribers':
      $subscriber_count = bwlb_get_list_subscriber_count($post_id);
      $export_href = bwlb_get_export_link($post_id);
...
// 5.13
// generate a csv file of subscriber data
// requires $_GET['list_id'] to be set in the url
function bwlb_download_subscribers_csv() {

  $list_id = ( isset($_GET['list_id']) ) ? (int) $_GET['list_id'] : 0;

  $csv = '';

  $list = get_post($list_id); // list object

  // get the subscribers for the list or all subscribers if no list provided
  $subscribers = bwlb_get_list_subscribers($list_id);

  if ($subscribers !== false):

    $now = new DateTime(); // for timestamp

    // generate filename
    $fn1 = 'bwlb-export_list-' . $list_id . '_' . $now->format('Ymd') . '.csv';
    $fn2 = plugin_dir_path(__FILE__) . 'exports/' . $fn1;

    $fp = fopen($fn2, 'w'); // open file in write mode

    $subscriber_data = bwlb_get_subscriber_data($subscribers[0]); // get the first subscriber

    // clear subscriptions and name from the data
    unset($subscribers['subscriptions']);
    ...
    // append the header row to the file
    ...
    foreach ($subscribers as &$subscriber_id):
      ...
      // append the remaining data to the csv file
    ...
    $fc = fread($fp, filesize($fn2));  // load csv content
...
function bwlb_check_wp_version() {

  global $pagenow; // wp variable with current filename

  if ( $pagenow == 'plugins.php' && is_plugin_active('bw-list-builder/bw-list-builder.php') ):

    $wp_version = get_bloginfo('version');

    // array of versions we've tested
    $tested_versions = array(
      '4.5.4',
      '4.6',    // no trailing zero
      '4.6.1'
    );

    if (!in_array($wp_version, $tested_versions)):

      $notice = bwlb_get_admin_notice('BW List Builder has not been tested in your version of WordPress, but it may still work.', 'error');
...
// 5.17
// wrapper for plugin uninstall
function bwlb_uninstall_plugin() {

  // custom tables
  bwlb_remove_plugin_tables();

  // custom post types, posts, and data
  bwlb_remove_post_data();
...
function bwlb_remove_plugin_tables() {
...
function bwlb_remove_post_data() {
  ...
  try {
    ...
    // array of the types to clean up
    ...
    // cleanup any orphaned meta data associated with the custom posts
    $wpdb->query(
      $wpdb->prepare(
        " DELETE pm
          FROM $pm_tablename pm
          LEFT JOIN $post_tablemane wp ON wp.ID = pm.post_id
          WHERE wp.ID IS NULL
        "
      )
    )
...
// 5.20
// remove custom plugin options
function bwlb_remove_options() {

  $options_removed = false;

  try {

    $options = bwlb_get_options_settings();

    foreach ($options['settings'] as &$setting):

      unregister_setting($options['group'], $setting); // wp builtin function
...
function bwlb_get_list_subscribers($list_id=0) {

  $subscribers = false;

  $list = get_post($list_id); // list object

  if (bwlb_validate_list($list_id)):

    // query all the subscribers from this list (post) only
  ...
  elseif ($list_id === 0):
    ...
    // no list specified, so return all subscribers
    $subscribers_query = new WP_Query(
      array(
        'post_type' => 'bwlb_subscriber',
        'published' => true,
  ...
  if ( isset($subscribers_query) && $subscribers_query->have_posts() ):
    ...
    while ($subscribers_query->have_posts()):
      ...
      $subscribers_query->the_post(); // sets current post object in memory
      $post_id = get_the_ID();  // get the id for the current post object
      array_push($subscribers, $post_id); // append the post object to the array

    endwhile;

  endif;

  // clear any data still in memory from the while loop
  wp_reset_query();
  wp_reset_postdata();
...
// 6.24
// return the number of subscribers for a list
function bwlb_get_list_subscriber_count($list_id=0) {
...
// 6.25
// return a unique link for downloading a subscribers csv
function bwlb_get_export_link($list_id=0) {
...
// 6.26
// return html for wp admin notices
function bwlb_get_admin_notice($message,$class) {
...
// 6.27
// provide an array of plugin options
function bwlb_get_options_settings() {

  $settings = array(

    'group' => 'bwlb_plugin_options',
    'settings' => array(
      'bwlb_manage_subscription_page_id',
      'bwlb_confirmation_page_id',
      'bwlb_reward_page_id',
      'bwlb_default_email_footer',
      'bwlb_download_limit'
    )
  );
...
function bwlb_dashboard_admin_page() {
  ...
  $export_href = bwlb_get_export_link();
...
function bwlb_register_options() {
  ...
  // rewrite to manage settings better programmatically vs hardcoding
  $options = bwlb_get_options_settings();
